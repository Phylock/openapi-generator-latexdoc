...
generators="${GEN_DIR}/extension/generators/"
...
if [[ -f "${codegen}" && -n "$(java ${JAVA_OPTS} -jar "${codegen}" completion | grep "^$1\$" )" ]]; then
    ...
    GENERATORS=
    for generator in ${generators}/*.jar; do
        GENERATORS="${GENERATORS}:${generator}"
    done

    exec java ${JAVA_OPTS} -cp "${codegen}${GENERATORS}" org.openapitools.codegen.OpenAPIGenerator "${command}" "$@"
